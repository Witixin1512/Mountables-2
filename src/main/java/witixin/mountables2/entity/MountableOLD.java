...
public class MountableOLD extends TamableAnimal implements IAnimatable, PlayerRideableJumping {
...
//            this.setFreeFly(getMountableData().aiModes()[2]);
//            this.setFreeSwim(getMountableData().aiModes()[0]);
//        }
//    }
...
//        this.goalSelector.addGoal(1, new MountableFloatGoal(this));
    ...
    public void refreshDimensions() {
        ...
        if (!this.level.isClientSide && !this.firstTick && !this.noPhysics && flag && (this.dimensions.width > preEventDim.width || this.dimensions.height > preEventDim.height)) {
            ...
            this.level.findFreePosition(this, voxelshape, vec3, this.dimensions.width, this.dimensions.height, this.dimensions.width).ifPresent((p_185956_) -> {
        ...
        if (this.isAlive()) {
            ...
            if (this.isVehicle() && this.canBeControlledByRider()) {
                ...
                if (this.isControlledByLocalInstance() && !this.getGroundMode().matches(GROUND_MOVEMENT.NONE.name())) {
                    float speed = this.isSlowOnLand() ? this.getSpeed() / 2 : this.getFlyingSpeed();
                    this.setSpeed(speed);
                    //Move the groundmode check to the caclulate speed shenanigans
                    super.travel(new Vec3(f, travelVec.y, f1));
                ...
                //AI MODE!
                float speed = this.isSlowOnLand() ? getSpeed() / 2 : this.getFlyingSpeed();
    ...
    private Vec3 doSlimeJump(float f1, double yScale, double force) {
        ...
        if (f1 > 0.0F && vector3d.y <= 0.3) {
            float f2 = Mth.sin(this.getYRot() * ((float) Math.PI / 180F));
            float f3 = Mth.cos(this.getYRot() * ((float) Math.PI / 180F));
            return (this.getDeltaMovement().add(-0.4F * f2 * force, 0.0D, 0.4F * f3 * force));
        }
...
//            PacketHandler.INSTANCE.send(PacketDistributor.PLAYER.with(() -> (ServerPlayer) pPlayer), new ClientOpenScreenPacket(getId()));
//            return InteractionResult.SUCCESS;
//        }
